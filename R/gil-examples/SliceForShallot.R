...
#  uni.slice.calls <<- uni.slice.calls + 1
  ...
  # Find the log density at the initial point, if not already known.
   ...
   uni.slice.evals[parmNumber] <<- uni.slice.evals[parmNumber] + 1
    ...
    for (i in 1:length(subset)){
      gxs <- gxs + jainneal.log.likelihood(subset[i],parameter)
    }
  ...
  u <- runif(1,0,w)
  L <- parameter[parmNumber] - u
  R <- parameter[parmNumber] + (w-u)  # should guarantee that x0 is in [L,R], even with roundoff
  ...
  { 
    ...
    { if (L<=lower) break
      ...
      gL <- 0
      for (i in 1:length(subset)){
        gL <- gL + jainneal.log.likelihood(subset[i],newParm)
      }
    ...
    { if (R>=upper) break
      ...
      gR <- 0
      for (i in 1:length(subset)){
        gR <- gR + jainneal.log.likelihood(subset[i],newParm)
      }
      if (gR<=logy) break
      R <- R + w
    }
  ...
  { 
    ...
    gx1 <- 0
    for (i in 1:length(subset)){
      gx1 <- gx1 + jainneal.log.likelihood(subset[i],newParm)
    }    
    if (gx1>=logy) break
    
    if (x1>parameter[parmNumber]) 
    { R <- x1
    }
    else 
    { L <- x1
...
if (length(parameter) == 1){
  ...
  newParm <- c(sub,parameter[2:length(parameter)])
}
else if (parmNumber == length(parameter)){
  newParm <- c(parameter[1:(length(parameter)-1)],sub)  
}
else {
  newParm <- c(parameter[1:(parmNumber-1)],sub,parameter[(parmNumber+1):length(parameter)])
}
}
